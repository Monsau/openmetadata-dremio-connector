...
class AgentRegistry:
    ...
    def get_available_agents(cls) -> List[Dict[str, Any]]:
        ...
        for agent_type, agent_class in cls._agents.items():
            # Retourner les métadonnées sans instancier (évite les erreurs de config)
            try:
                # Extraire description du docstring
                description = ""
                if agent_class.__doc__:
                    lines = [l.strip() for l in agent_class.__doc__.split('\n') if l.strip()]
                ...
                logger.warning(f"Erreur récupération métadonnées {agent_type}: {str(e)}")
