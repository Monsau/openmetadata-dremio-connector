...
class DremioClient:
	...
	def discover_all_resources(self) -> List[Dict[str, Any]]:
		"""
		Découvre toutes les ressources Dremio (sources + datasets).
		Mode FULL synchronization.
		
		Returns:
			Liste de toutes les ressources avec leurs métadonnées
		"""
		logger.info("🔍 Mode FULL: Découverte complète des ressources Dremio...")
		resources = []
		
		try:
			# 1. Récupérer toutes les sources
			sources = self.get_sources()
			logger.info(f"  ✓ {len(sources)} sources découvertes")
			
			# 2. Pour chaque source, récupérer ses datasets
			for source in sources:
				...
				# Récupérer datasets de cette source
				datasets = self.get_source_datasets(source['id'])
				logger.info(f"  ✓ {len(datasets)} datasets dans {source['name']}")
				
				for dataset in datasets:
					# Récupérer schéma du dataset
			...
			# 3. Récupérer aussi les VDS (espaces utilisateurs)
			...
			if vds_list:
				...
				logger.info(f"  ✓ {len(vds_list)} VDS découverts")
			
			logger.info(f"✅ Total: {len(resources)} ressources principales découvertes")
			return resources
			
		except Exception as e:
			logger.error(f"❌ Erreur discover_all_resources: {e}")
			return []
	
	def discover_incremental_resources(self, since_timestamp: Optional[str] = None) -> List[Dict[str, Any]]:
		"""
		Découvre les ressources Dremio modifiées depuis un timestamp.
		...
		"""
		logger.info("🔍 Mode INCREMENTAL: Découverte des modifications Dremio...")
		
		if not since_timestamp:
			logger.info("  ⚠️  Pas de timestamp fourni, mode FULL activé")
			return self.discover_all_resources()
		
		# Pour l'instant, implémentation simple qui retourne tout
		# TODO: Filtrer par timestamp quand API Dremio le supporte
		logger.info(f"  📅 Depuis: {since_timestamp}")
		logger.info("  ⚠️  Filtrage par date non encore implémenté, retourne tout")
